...
mkdir -p \
  ...
  "$OUT_DIR/JsonMapping" \
  "$OUT_DIR/PlaceAndRoute"

# =======================================
# [1/5] Elaboration (Yosys)
# =======================================

echo "[1/5] Elaboration (Yosys) …"
...
# =======================================
# [2/5] ABC cleansing (clean AIG voor Mockturtle)
# =======================================

echo "[2/5] ABC: strash + write_aiger (clean AIG) …"
...
# =======================================
# [3/5] Logic Synthesis (Mockturtle)
# =======================================

echo "[3/5] Logic synthesis (Mockturtle) op clean .aig …"
...
# ---------------------------------------
# [3a/5] Equivalence check AIG vóór/na Mockturtle
# ---------------------------------------

echo "[3a/5] Equivalence check (mt_aig_equiv) …"
...
if ! "$MT_AIG_EQUIV_BIN" "$CLEAN_AIG" "$POSTOPT_AIG" >"$AIG_EQUIV_LOG" 2>&1; then
  ...
  echo "[3a/5] Equivalence check geslaagd: CLEAN_AIG ≡ POSTOPT_AIG"
fi

# =======================================
# [3b/5] Export LUT→AIG cones (mt_lut_cones)
# =======================================

echo "[3b/5] Export LUT→AIG cones (mt_lut_cones) …"
...
[[ -s "$LUT_CONES_JSON" ]] || fail "LUT JSON niet gegenereerd: $LUT_CONES_JSON"
[[ -s "$MAPPED_BLIF"    ]] || fail "Mapped BLIF niet gegenereerd: $MAPPED_BLIF"

# =======================================
# [3c/5] Genereer LUT Boolean expressions (DNF)
# =======================================

echo "[3c/5] Build LUT Boolean expressions (DNF) ..."
...
echo "[3c/5] LUT Boolean expressions geschreven naar: $LUT_BOOL_EXP_JSON"

# =======================================
# [4/5] Placement & Routing (VPR) + analyses
# =======================================
...
if [ -d "$VPR_SRC_DIR" ]; then
  ...
  if [ -x "$ORG_SCRIPT" ]; then
    "$ORG_SCRIPT" --dir "$VPR_DEST_DIR" || echo "Waarschuwing: organiseren van VPR outputs faalde" >>"$VPR_LOG"
  else
    echo "Waarschuwing: organize_vpr_outputs.sh niet uitvoerbaar of niet gevonden" >>"$VPR_LOG"
  fi
else
  echo "Waarschuwing: VPR bronmap niet gevonden: $VPR_SRC_DIR" >>"$VPR_LOG"
fi

# ---------------------------------------
# [4a] PRE-ROUTING Manhattan analyse (place-only)
# ---------------------------------------

echo "[4a] Pre-routing placement analyse (Manhattan) ..."
...
echo "[4a] Manhattan JSON + Top20 verbindingen gegenereerd."

# ---------------------------------------
# [4b] Mid-LUT analyse + selectie
# ---------------------------------------

echo "[4b] Mid-LUT analyse (kandidaten zoeken) ..."
...
echo "[4b] Mid-LUT kandidaten geschreven naar: $MID_LUTS_JSON"
...
echo "[4b] Beste mid-LUT combinaties geschreven naar: $BEST_MID_LUTS_JSON"

# ---------------------------------------
# [4c] Build LUT connections JSON
# ---------------------------------------

echo "[4c] Build LUT connections JSON ..."

[[ -f "$LUT_CONN_SCRIPT" ]] || fail "build_lut_connection_aig_json.py niet gevonden: $LUT_CONN_SCRIPT"

MANHATTAN_JSON="$PRE_CUSTOM_DIR/${design}.manhattan.json"
...
[[ -s "$LUT_BOOL_EXP_JSON"   ]] || fail "Ontbreekt: $LUT_BOOL_EXP_JSON"
...
echo "[4c] LUT connections json geschreven naar: $LUT_CONNS_JSON"

# ---------------------------------------
# [4d] Build SUPER json voor SAT
# ---------------------------------------
...
MAPPED_NET="$PRE_LOC/${design}.mapped.net"
