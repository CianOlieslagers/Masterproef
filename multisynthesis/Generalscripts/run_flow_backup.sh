...
POSTOPT_AAG="$OUT_DIR/LogicSynthesis/Aig/${MT_DESIGN}.postopt.aag"

"$YOSYS_BIN" -p "
read_aiger $POSTOPT_AIG
write_aiger -ascii $POSTOPT_AAG
" > "$LOGS_DIR/${design}.yosys.postopt_aag.log" 2>&1

[[ -s "$POSTOPT_AAG" ]] || fail "Yosys kon geen postopt .aag genereren: $POSTOPT_AAG"
...
# =======================================
# Stap 3c — Genereer LUT Boolean expressions (build_lut_boolean_expressions.py)
# =======================================

echo "[3c] Build LUT Boolean expressions (DNF) ..."

LUT_BOOL_SCRIPT="${LUT_BOOL_SCRIPT:-$HOME/Masterproef/multisynthesis/Lut_verbinding/script/build_lut_boolean_expressions.py}"
[[ -f "$LUT_BOOL_SCRIPT" ]] || fail "build_lut_boolean_expressions.py niet gevonden: $LUT_BOOL_SCRIPT"

# Output in dezelfde result-structuur als je andere LUT_verbinding outputs
LUT_BOOL_OUT_DIR="$OUT_DIR/JsonMapping"
mkdir -p "$LUT_BOOL_OUT_DIR"

LUT_BOOL_EXP_JSON_DEFAULT="$LUT_BOOL_OUT_DIR/${design}.lutBooleanExp.json"
LUT_BOOL_EXP_JSON="${LUT_BOOL_EXP_JSON:-$LUT_BOOL_EXP_JSON_DEFAULT}"

LUT_BOOL_LOG="$LOGS_DIR/${design}.lutBooleanExp.log"

[[ -s "$LUT_JSON" ]] || fail "Ontbreekt: $LUT_JSON"

"$PYTHON" "$LUT_BOOL_SCRIPT" \
  --lut-cones "$LUT_JSON" \
  --out "$LUT_BOOL_EXP_JSON" \
  > "$LUT_BOOL_LOG" 2>&1

[[ -s "$LUT_BOOL_EXP_JSON" ]] || fail "lutBooleanExp.json niet gegenereerd: $LUT_BOOL_EXP_JSON"

echo "[3c] LUT Boolean expressions geschreven naar: $LUT_BOOL_EXP_JSON"


# =======================================
# Stap 4c.5 — Build LUT connections JSON (build_lut_connection_aig_json.py)
# =======================================

echo "[4c.5] Build LUT connections JSON ..."

LUT_CONN_SCRIPT="${LUT_CONN_SCRIPT:-$HOME/Masterproef/multisynthesis/Lut_verbinding/script/build_lut_connection_aig_json.py}"
[[ -f "$LUT_CONN_SCRIPT" ]] || fail "build_lut_connection_aig_json.py niet gevonden: $LUT_CONN_SCRIPT"

# Inputs (bestaan nu al)
MID_LUTS_JSON="$OUT_DIR/PlaceAndRoute/05_custom_outputs/${design}.mid_luts.json"
MANHATTAN_JSON="$OUT_DIR/PlaceAndRoute/05_custom_outputs/${design}.manhattan.json"
LUT_CONES_JSON="$OUT_DIR/JsonMapping/${design}.lut_cones.json"

[[ -s "$MID_LUTS_JSON" ]]     || fail "Ontbreekt: $MID_LUTS_JSON"
[[ -s "$MANHATTAN_JSON" ]]    || fail "Ontbreekt: $MANHATTAN_JSON"
[[ -s "$LUT_CONES_JSON" ]]    || fail "Ontbreekt: $LUT_CONES_JSON"

# Output: jouw bestaande structuur behouden
LUT_CONN_OUT_DIR="$OUT_DIR/JsonMapping"
mkdir -p "$LUT_CONN_OUT_DIR"

LUT_CONNS_JSON_DEFAULT="$LUT_CONN_OUT_DIR/${design}.lut_connections_full.json"
LUT_CONNS_JSON="${LUT_CONNS_JSON:-$LUT_CONNS_JSON_DEFAULT}"

# Boolean expressions (voorlopig hardcoded zoals je vroeg)
LUT_BOOL_EXP_JSON_DEFAULT="$OUT_DIR/JsonMapping/${design}.lutBooleanExp.json"
LUT_BOOL_EXP_JSON="${LUT_BOOL_EXP_JSON:-$LUT_BOOL_EXP_JSON_DEFAULT}"

# Log
LUT_CONN_LOG="$LOGS_DIR/${design}.lut_connections_full.log"

# Boolean exp is optioneel, maar jij wil hem nu altijd meegeven en hardcoden
[[ -s "$LUT_BOOL_EXP_JSON" ]] || fail "Ontbreekt (hardcoded lutBooleanExp): $LUT_BOOL_EXP_JSON (zet LUT_BOOL_EXP_JSON om te overrulen)"

"$PYTHON" "$LUT_CONN_SCRIPT" \
  --mid-luts "$MID_LUTS_JSON" \
  --lut-cones "$LUT_CONES_JSON" \
  --manhattan "$MANHATTAN_JSON" \
  --lut-bool-exp "$LUT_BOOL_EXP_JSON" \
  --out "$LUT_CONNS_JSON" \
  > "$LUT_CONN_LOG" 2>&1

[[ -s "$LUT_CONNS_JSON" ]] || fail "LUT connections json niet gegenereerd: $LUT_CONNS_JSON"

echo "[4c.5] LUT connections json geschreven naar: $LUT_CONNS_JSON"


# =======================================
# Stap 4d — Build SUPER json voor SAT (build_super_json_sat.py)
# =======================================

echo "[4d] Build SUPER SAT json ..."

SUPER_SCRIPT="${SUPER_SCRIPT:-$HOME/Masterproef/multisynthesis/Lut_verbinding/script/build_super_json_sat.py}"
[[ -f "$SUPER_SCRIPT" ]] || fail "build_super_json_sat.py niet gevonden: $SUPER_SCRIPT"

# Inputs uit jouw flow
MAPPED_NET="$OUT_DIR/PlaceAndRoute/01_location/${design}.mapped.net"
LUT_CONES="$OUT_DIR/JsonMapping/${design}.lut_cones.json"
POSTOPT_AAG="$OUT_DIR/LogicSynthesis/Aig/${MT_DESIGN}.postopt.aag"
MAPPED_BLIF="$OUT_DIR/topologymapping/${design}.mapped.blif"

# Connections JSON: default naar jouw bestaande result-structuur
LUT_CONNS_JSON_DEFAULT="$OUT_DIR/JsonMapping/${design}.lut_connections_full.json"
LUT_CONNS_JSON="${LUT_CONNS_JSON:-$LUT_CONNS_JSON_DEFAULT}"

SUPER_OUT="$OUT_DIR/JsonMapping/${design}.super.sat.v2.json"
SUPER_LOG="$LOGS_DIR/${design}.superjson.log"

[[ -s "$MAPPED_NET" ]]  || fail "Ontbreekt: $MAPPED_NET"
[[ -s "$LUT_CONES"  ]]  || fail "Ontbreekt: $LUT_CONES"
[[ -s "$POSTOPT_AAG" ]] || fail "Ontbreekt: $POSTOPT_AAG"
[[ -s "$MAPPED_BLIF" ]] || fail "Ontbreekt: $MAPPED_BLIF"
[[ -s "$LUT_CONNS_JSON" ]] || fail "Ontbreekt LUT connections json: $LUT_CONNS_JSON (zet LUT_CONNS_JSON of genereer dit bestand)"

"$PYTHON" "$SUPER_SCRIPT" \
  --mapped-net "$MAPPED_NET" \
  --lut-cones "$LUT_CONES" \
  --conns "$LUT_CONNS_JSON" \
  --postopt-aag "$POSTOPT_AAG" \
  --mapped-blif "$MAPPED_BLIF" \
  --out "$SUPER_OUT" \
  > "$SUPER_LOG" 2>&1

[[ -s "$SUPER_OUT" ]] || fail "SUPER json niet gegenereerd: $SUPER_OUT"

echo "[4d] SUPER SAT json geschreven naar: $SUPER_OUT"
