...
OUT_DIR="$ECO_ROOT/phase6b_route_patched_no_absorb"
rm -rf "$OUT_DIR"
mkdir -p "$OUT_DIR"
...
echo "[OK] Route-only patched ECO gelukt met --absorb_buffer_luts off."
echo "[OK] Route file: $ROUTE"
echo "[OK] Log file  : $LOG"

echo
echo "[INFO] Snelle log-check:"
grep -Ei "Absorbed|Packer:|Placer:|Router:|Load packing|Load Placement|Routing|Successfully routed|routing consistency|error|failed|mismatch|Fixed|ECO_BUF" "$LOG" | tail -160 || true
